      ...
      *
      * ファイル定義
     FPERSON    UF A E           K DISK    INFDS(dbfds)
     F                                     INFSR(#DBEX)
     FEFADSPF   CF   E             WORKSTN INDDS(ws)
      *
      * サブプロシージャー定義
     DsetMsg           PR
     D  msgType                       1    CONST
      ...
      /FREE
        ...
        *INLR = *ON;
        ...
        BEGSR #MOVE;
          ...
          // 初期画面の場合は部分キーで位置づけ
          IF (NOT ws.dspDetail_60) AND (REGNO <> 0);
            SETLL(E) REGNO PERSONR;
            EXSR #DBEX; // ＤＢエラー
          ENDIF;
          ...
          // 最初・最後のレコードを超えた→ MSG 表示
          IF %EOF;
            IF ws.prev_07;
              SETLL(E) *HIVAL PERSONR;
            ELSE; // ws.next_08
              SETLL(E) *LOVAL PERSONR;
            ENDIF;
            EXSR #DBEX; // ＤＢエラー
        ...
        BEGSR #ADD;
          ...
          DOW (1 = 1);
            ...
            // キー未入力エラー→エラー表示、明細表示
            IF (NOT ws.errorMsg_50) AND (REGNO = 0);
              setMsg('E' : ' 登録番号が未入力。 ');
            ENDIF;
        ...
        BEGSR #CHAIN;
          ...
          CHAIN(E) REGNO PERSONR;

          EXSR #DBEX; // ＤＢエラー
          // キーなしエラー→メッセージ表示、明細非表示
          IF NOT %FOUND;
            setMsg('E' : ' レコードが見つからない。 ');
            ws.dspDetail_60 = *OFF;   // 明細非表示
          // キーあり→明細表示
          ELSE;
            ws.dspDetail_60 = *ON;    // 明細表示
